...
class Solution:
    ...
    def __get_index_of_target(self, nums: List[int], target: int, left: int, right: int) -> int:
        if left <= right:
            middle = left + (right - left) // 2

            if nums[middle] == target: return middle
            elif nums[middle] < target:
                if (nums[left] <= nums[middle]) or ((nums[left] > nums[middle]) and (nums[left] > target)):
                    return self.__get_index_of_target(nums, target, middle + 1, right)
                else:
                    return self.__get_index_of_target(nums, target, left, middle - 1)
            else:
                if ((nums[left] < nums[middle]) and (nums[left] <= target)) or ((nums[middle] < nums[right]) and (nums[right] > target)):
                    return self.__get_index_of_target(nums, target, left, middle - 1)
                else:
                    return self.__get_index_of_target(nums, target, middle + 1, right)
        else: return -1
