...
private:
  int returnIndexOfValue(const std::vector<int> &nums, int left, int right,
                         int target) {
    if (left > right)
      return -1;
    else {
      ...
      if (nums[middle] == target)
        ...
        return returnIndexOfValue(nums, middle + 1, right, target);
      else
        return returnIndexOfValue(nums, left, middle - 1, target);
    }
  }
};
