  ...
  int shortestBridge(const std::vector<std::vector<int>> &grid) {
    ...
    for (int i = 0; i < static_cast<int>(firstIsland.size()); i++)
      for (int j = 0; j < static_cast<int>(secondIsland.size()); j++)
        minimumFlip = std::min(
            minimumFlip,
            abs(firstIsland[i].first - secondIsland[j].first) +
                abs(firstIsland[i].second - secondIsland[j].second) - 1);
...
private:
  ...
  void setMarks(int i, int j, int number, std::vector<std::vector<int>> &grid) {
    if ((i > -1) && (i < static_cast<int>(grid.size())) && (j > -1) &&
        (j < static_cast<int>(grid[0].size())) && (grid[i][j] == 1)) {
      grid[i][j] = number;

      if (number == 2)
        firstIsland.push_back({i, j});
      else
        secondIsland.push_back({i, j});
