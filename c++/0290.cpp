...
public:
  bool wordPattern(const std::string &pattern, const std::string &s) {
    ...
    if (static_cast<int>(pattern.size()) != static_cast<int>(words.size()))
      return false;

    int i = 0;
    bool isCorrect = true;
    while ((i < static_cast<int>(pattern.size())) && (isCorrect)) {
      if ((relationLetterToWord.find(pattern[i]) ==
           relationLetterToWord.end()) &&
...
private:
  ...
  void getAllWords(const std::string &source,
