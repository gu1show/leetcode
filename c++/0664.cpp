...
public:
  int strangePrinter(const std::string &s) {
    std::vector<std::vector<int>> segmentsChanging(
        s.size(), std::vector<int>(s.size(), s.size()));
    for (int length = 1; length <= static_cast<int>(s.size()); length++) {
      for (int left = 0; left <= static_cast<int>(s.size() - length); left++) {
        ...
        for (int i = left; i < right; i++) {
          ...
          if (j != -1)
            segmentsChanging[left][right] = std::min(
                segmentsChanging[left][right],
                1 + segmentsChanging[j][i] + segmentsChanging[i + 1][right]);
        }

        if (j == -1)
          segmentsChanging[left][right] = 0;
      }
    }
