...
public:
  bool canFinish(int numCourses,
    ...
    while ((i < numCourses) && (canFinishAllCourses)) {
      if (dfs(i, adjacentNodes, isVisited, isInStack))
        canFinishAllCourses = false;
      i++;
    }
...
private:
  bool dfs(int courseNumber, const std::vector<std::vector<int>> &adjacentNodes,
           std::vector<bool> &isVisited, std::vector<bool> &isInStack) {
    if (isInStack[courseNumber])
      return true;
    else if (isVisited[courseNumber])
      return false;
