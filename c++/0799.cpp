...
public:
    ...
    {
        std::vector<std::vector<double>> champagneTowerFilling(101, std::vector<double>(101));
        champagneTowerFilling[0][0] = poured;
        for (int row = 0; row <= query_row; row++)
        {
            for (int column = 0; column <= query_glass; column++)
            {
                double pouringOutEachSide = (champagneTowerFilling[row][column] - 1) / 2.0;
...
public:
    double champagneTower(int poured, int query_row, int query_glass) 
    {
        ...
        for (int row = 0; row < query_row; row++)
        {
            ...
            for (int column = 0; column < static_cast<int>(levelOfChampagneTower.size()); column++)
            {
