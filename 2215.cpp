...
public:
    std::vector<std::vector<int>> findWinners(const std::vector<std::vector<int>>& matches) 
    {
        ...
        {
            if (val.second == 0) zeroAndOneLosses[0].push_back(key);
            else if (val.second == 1) zeroAndOneLosses[1].push_back(key);
        }
...
private:
    ...
    void updateTable(int participant, bool isWin) 
    {
        ...
        else
        {
            ...
            if (isWin) score = std::make_pair(1, 0);
            else score = std::make_pair(0, 1);
            scoreTable[participant] = score;
        }
    }
};
